...
| Element | Wert / Zweck |
|---------|--------------|
| Stützstellen `N` | 125 (grob) bzw. bis 220 (fein), je nach Neustart |
| Horizont `T` | `2,5 s · 4,22 / ω_hang`, begrenzt auf 1,5–9,0 s, je nach Neustart zusätzlich ×1,5 oder ×0,75 |
...
schwach beschleunigte Pendel proportional mehr Zeit bekommen.
...
austherapiert; die letzten sechs Versuche einer Runde laufen dagegen immer
voll durch. Die Neustarts durchlaufen vier Kombinationen aus Horizont und
Auflösung — 125 oder bis zu 220 Stützstellen, Horizont ×1, ×1,5 oder ×0,75.
Keine Wahl gewinnt überall: das grobe Gitter konvergiert bei den
Standardparametern häufiger, sehr lange Glieder liefern brauchbare Bahnen
nur auf dem feinen, und bei kleinem `g` gelingt es erst mit gestrecktem
Horizont. Da jede Kandidatin ohnehin durchsimuliert wird (Abschnitt 5.3),
kann zusätzliche Vielfalt hier nur helfen.
...
2. anschließend das Vierfache des Planungshorizonts mit dem balancierenden
   LQR, inklusive Rückführung der Wagen-Sollposition. Die Fensterlänge muss
   der Strecke folgen: bei `g = 1 m/s²` ist der Regelkreis ein Vielfaches
   langsamer als bei 9,81, ein festes Fenster würde brauchbare Bahnen
   verwerfen, nur weil sie noch nicht fertig eingeschwungen sind.
...
Schienenanschlag), startet im Hintergrund eine weitere Suchrunde mit je 20
Neustarts, während der Regler erneut beruhigt — bis zu zehn Runden.
...
Schrittweiten um 20 ms gerechnet, die Simulation läuft mit 62,5 µs. Gemessene
